...
impl Scene {
    ...
    pub fn take_recording(&mut self) -> Option<Self> {
        ...
        core::mem::swap(&mut self.recorder, &mut recording.recorder);
        core::mem::swap(&mut self.encoded_paints, &mut recording.encoded_paints);
        core::mem::swap(
    ...
    fn recording_is_closed(&self) -> bool {
        !self.has_open_layers() && self.viewport_state.clip().is_none()
    }
    ...
    pub fn try_append(&mut self, other: &mut Self) -> bool {
        ...
        if let (Some(last), Some(first)) = (target.nodes.last_mut(), source.nodes.first())
            && last.layer.is_none()
            && last.draws.end == first.draws.start
        {
            last.draws.end = first.draws.end;
...
fn append_vec<T>(target: &mut alloc::vec::Vec<T>, source: &mut alloc::vec::Vec<T>) {
